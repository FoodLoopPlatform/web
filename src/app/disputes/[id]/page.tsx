...
import {
  ...
  getMySupportTickets,
} from "@/app/disputes/api/support-tickets-api";
import { DisputeDetailContent } from "@/app/disputes/components/DisputeDetailContent";
import { DisputeDetailSkeleton } from "@/app/disputes/components/DisputeDetailSkeleton";

interface DisputeDetailPageProps {
  params: Promise<{ id: string }>;
}

function DisputeDetailPage({ params }: DisputeDetailPageProps) {
  const { id } = use(params);
  const store = useStoreProfile();
  const [mobileSidebarOpen, setMobileSidebarOpen] = useState(false);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);

  const ticketPromise = useMemo(() => getSupportTicketById(id), [id]);
  const otherTicketsPromise = useMemo(() => getMySupportTickets(), []);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Dispute Detail Section with Suspense Boundary */}
        <section className="flex-1 p-margin-mobile md:p-margin-desktop bg-surface-container-lowest overflow-y-auto">
          <Suspense fallback={<DisputeDetailSkeleton />}>
            <DisputeDetailContent
              ...
              otherTicketsPromise={otherTicketsPromise}
            />
          </Suspense>
        </section>
...
export default withAuth(DisputeDetailPage);
