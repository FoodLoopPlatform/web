...
import { ModerationConfidenceBadge, ModerationFlagBadge } from "./ModerationBadge";
...
export const ModerationListingCard: React.FC<ModerationListingCardProps> = ({
  ...
  const productName = isRtl ? item.productNameAr || item.productName : item.productNameEn || item.productName;
  const storeName = isRtl ? item.storeNameAr || item.storeName : item.storeNameEn || item.storeName;
  const flagReasonQuote = isRtl ? item.flagReasonQuoteAr || item.flagReasonQuote : item.flagReasonQuoteEn || item.flagReasonQuote;
      ...
      <div className="relative w-full h-36 sm:h-40 rounded-lg overflow-hidden bg-surface-container-high shrink-0 border border-outline-variant/40">
        {!imgError && item.imageUrl ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center bg-surface-container-high text-outline gap-1">
            <svg className="w-10 h-10 opacity-50 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            <span className="text-[10px] font-bold text-outline">{isRtl ? "صورة المنتج" : "Product Image"}</span>
          </div>
        )}
