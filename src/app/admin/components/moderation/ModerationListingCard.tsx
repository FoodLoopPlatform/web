...
interface ModerationListingCardProps {
  ...
  onCardClick?: (item: ModerationItem) => void;
...
export const ModerationListingCard: React.FC<ModerationListingCardProps> = ({
  ...
  return (
    ...
    >
      {/* Clickable Card Header & Content Body */}
      <div
        onClick={() => onCardClick?.(item)}
        className="flex flex-col gap-3 cursor-pointer group"
      >
        {/* Product Image Container & AI Badge */}
        <div className="relative w-full h-36 sm:h-40 rounded-lg overflow-hidden bg-surface-container-high shrink-0 border border-outline-variant/40 group-hover:border-primary/50 transition-colors">
        ...
        <div className="flex flex-col gap-0.5">
          <h3 className="text-sm font-bold text-on-surface tracking-tight leading-snug group-hover:text-primary transition-colors">
            {productName}
          </h3>
