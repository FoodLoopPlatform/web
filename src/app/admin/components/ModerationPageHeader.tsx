...
export const ModerationPageHeader: React.FC<ModerationPageHeaderProps> = ({
  ...
  const countText = t.pendingItemsCount.replace(
    "{count}",
    String(pendingCount),
  );
  ...
  return (
    <div
      className={`flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-outline-variant/60 pb-5 ${isRtl ? "text-right" : "text-left"}`}
    >
      {/* Eyebrow & Title */}
      ...
      <div className="flex items-center gap-3 self-start md:self-end">
        ...
        <div className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-tertiary-fixed/70 text-on-tertiary-fixed-variant border border-tertiary-fixed-dim font-bold text-xs shadow-2xs">
          <svg
            className="w-4 h-4 text-on-tertiary-fixed-variant shrink-0"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <button
          ...
          className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-xs text-on-primary transition-all shadow-xs cursor-pointer active:scale-95 ${
            isFilterActive
              ? "bg-primary-container ring-2 ring-primary-fixed-dim"
              : "bg-primary hover:bg-primary-container"
          }`}
        >
          <svg
            className="w-4 h-4 text-on-primary shrink-0"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
