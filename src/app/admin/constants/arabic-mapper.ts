...
const ARABIC_MAPPINGS: Record<string, string> = {
  ...
  "Potential health hazard claim review required": "ادعاء وجود خطر صحي - يتطلب المراجعة",

  // Review Comments
  "The donuts box was squashed and some donuts were missing.": "كانت علبة الدوناتس مضغوطة وكان هناك بعض القطع مفقودة.",
  "Sold me dairy products that expired 2 days ago! Unacceptable.": "تم بيع منتجات ألبان منتهية الصلاحية منذ يومين! أمر غير مقبول.",
  "Excellent bag! Got a huge variety of cheese and baked goods for only 90 EGP instead of 300.": "حقيبة ممتازة! حصلت على تشكيلة كبيرة من الأجبان والمخبوزات بـ 90 ج.م فقط بدلاً من 300.",

  // Support Tickets
  "Payment failure during checkout": "فشل عملية الدفع عند الخروج",
  "I tried purchasing a food loop bag from El Abd Bakery, but the app crashed on the payment page and the money was deducted from my InstaPay, but no order was generated.": "حاولت شراء حقيبة فود لوب من مخبز العبد، ولكن التطبيق توقف عند صفحة الدفع وتم خصم المبلغ من إنستاباي دون إنشاء الطلب.",
  "Unable to update inventory": "غير قادر على تحديث المخزون",
  "The bulk inventory upload feature is throwing a 500 Server Error when we upload our Excel spreadsheet of surplus products.": "خاصية رفع المخزون بالجملة تعرض خطأ 500 بالسيرفر عند رفع ملف الإكسيل للمنتجات الفائضة.",
  "Here is the transaction screenshot: Instapay ref #339102": "إليك صورة عملية الدفع: مرجع إنستاباي #339102",
  "Hello! We are looking into your upload template. Please ensure the dates are formatted as YYYY-MM-DD.": "أهلاً بك! نحن نراجع نموذج الرفع الخاص بك. يرجى التأكد من تنسيق التواريخ كـ YYYY-MM-DD.",
  "Thanks, we confirmed they are in that format. What else can it be?": "شكراً، تأكدنا أنها بهذا التنسيق. ماذا يمكن أن تكون المشكلة أيضاً؟",
 ...
 */
export function arText(text: string | undefined | null, isRtl: boolean = true): string {
  if (!text) return "";
