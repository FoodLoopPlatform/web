...
export default function SystemSettingsPage() {
  ...
  const admins = [
    {
      name: t.mainController,
      email: "admin@foodloop.eg",
      role: t.seniorControllerRole,
      status: t.active,
    },
    {
      name: t.sarahAdmin,
      email: "sarah@foodloop.eg",
      role: t.controllerRole,
      status: t.active,
    },
    {
      name: t.mikeAdmin,
  ...
  return (
    <div className="flex flex-col gap-6 lg:gap-8 w-full max-w-[1200px] mx-auto pb-12">
      ...
      {toastMessage && (
        <div
          className={`fixed bottom-6 ${lang === "ar" ? "right-6" : "left-6"} bg-primary-container border border-on-primary-container text-white px-5 py-3.5 rounded-xl shadow-lg z-50 text-xs font-bold animate-in slide-in-from-bottom duration-250 flex items-center gap-2`}
        >
          <svg
      ...
      <div className={lang === "ar" ? "text-right" : "text-left"}>
        <h1 className="text-2xl sm:text-3xl font-bold font-brand tracking-tight text-primary serif-ish">
          {t.title}
        </h1>
        <p className="text-xs sm:text-sm text-outline mt-1 sm:mt-1.5 font-medium">
          {t.subtitle}
        </p>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-6 lg:gap-8">
          ...
          <div className="bg-white rounded-2xl border border-card-border p-6 shadow-sm flex flex-col gap-4">
            ...
            <div className="divide-y divide-surface-container mt-2">
              <div
                className={`py-4 flex justify-between items-center ${lang === "ar" ? "flex-row" : "flex-row-reverse"}`}
              >
                <div
                  className={`flex flex-col gap-0.5 ${lang === "ar" ? "items-start text-right" : "items-end text-left"}`}
                >
          ...
          <div className="bg-white rounded-2xl border border-card-border p-6 shadow-sm flex flex-col gap-4">
            <div className={lang === "ar" ? "text-right" : "text-left"}>
              ...
              <span className="text-[10px] text-outline font-medium block mt-0.5">
                {t.operationalParamsSub}
              </span>
            </div>

            <form
              onSubmit={handleSaveConfig}
              className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-2 text-xs font-medium"
            >
              <div className={lang === "ar" ? "text-right" : "text-left"}>
                <label className="block text-on-surface-variant mb-1.5 font-bold uppercase tracking-wider">
                  {t.commissionLabel}
                </label>
                <input
                  type="number"
                  required
                  className={`w-full px-3 py-2.5 rounded-lg border border-outline-variant focus:outline-none focus:ring-1 focus:ring-surface-tint focus:border-surface-tint bg-surface ${
              ...
              <div className={lang === "ar" ? "text-right" : "text-left"}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setThresholds({ ...thresholds, rateLimit: e.target.value })
                  }
                />
              </div>
              ...
              <button
                type="submit"
                className="col-span-1 sm:col-span-2 bg-primary-container hover:bg-primary text-white py-3 rounded-lg font-bold transition-all text-xs cursor-pointer shadow-sm active:scale-95 mt-2"
              >
                {t.saveBtn}
              </button>
            </form>
          ...
          <div className="bg-white rounded-2xl border border-card-border p-6 shadow-sm flex flex-col gap-4">
            ...
            <div className="overflow-x-auto mt-2">
              <table
                className={`w-full border-collapse ${lang === "ar" ? "text-right" : "text-left"}`}
              >
                <thead>
                ...
                <tbody className="divide-y divide-surface-container">
                  {admins.map((adm, idx) => (
                    ...
                    >
                      <td className="px-4 py-3.5 text-xs font-bold text-on-surface whitespace-nowrap">
                        {adm.name}
                      </td>
                      <td className="px-4 py-3.5 text-xs text-outline whitespace-nowrap">
                        {adm.email}
                      </td>
                      <td className="px-4 py-3.5 text-xs whitespace-nowrap">
                        <span className="text-[9px] bg-surface-container text-primary px-2.5 py-0.5 rounded font-extrabold uppercase tracking-wider">
                          {adm.role}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-xs text-green-600 font-extrabold whitespace-nowrap">
                        ● {adm.status}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-primary-container text-white p-6 rounded-2xl flex flex-col justify-between min-h-[220px] shadow-elevation-2 relative overflow-hidden">
            ...
            <button
              ...
              className={`mt-6 bg-error hover:bg-red-700 text-white font-bold text-xs py-2.5 px-4 rounded-xl transition-all cursor-pointer active:scale-95 z-10 border border-red-500 ${
          ...
          <div className="bg-white rounded-2xl border border-card-border p-6 shadow-sm">
            ...
            <p className="mt-4 text-center text-xs text-outline py-6">
              {lang === "ar"
                ? "لا يتوفر سجل إعدادات حالياً."
                : "No settings logs available yet."}
            </p>
          </div>
        </div>
      </div>
